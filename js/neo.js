...
var Neo = function(_arguments) {
    ...
    this.matchQuery = function(query, element) {
        ...
        } else if(global.isDefined(element.matchesSelector)) {
            return element.matchesSelector(query);
        } else {
            return [].indexOf.call(document.querySelectorAll(query), element) !== -1;
        }
    ...
    var node = function(_origin) {
        ...
        /**
         * Trigger an interaction event of the node.
         * @param _events The interaction events.
         * @returns {node}
         */
        this.trigger = function(_events) {

            var _trigger = function(_event) {
                if(_event in self.events) {
                    for(var key in self.events[_event]) {
                        self.events[_event][key](window.event || { preventDefault: function() {} });
                    }
                }
            };
        ...
        this.parent = function(selector) {
            ...
            if(global.isDefined(selector)) {

                var _parent = self.origin;

                while(global.isNotNull(_parent = _parent.parentElement)) {
                    if(global.matchQuery(selector, _parent)) {
                        return global.select(_parent);
                    }
                }
            ...
            return global.select(self.origin.parentElement);
         ...
         * @returns {*}
         */
        this.frameWindow = function() {
            return global.select(self.origin.contentWindow);
        };
